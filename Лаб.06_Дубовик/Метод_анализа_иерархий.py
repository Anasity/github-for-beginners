        ...
        for j in range(n):
            ...
            if (i < j):
                while table[i][j] == 0: #для выхода из цикла ввода элемента ожидается верный ввод
                    ...
                    if (ratio == -1) or (a in (0.5,0.33,0.25,0.2,0.14,0.125,0.11) == False and (1 <= ratio <= 9) == False): #проверка введённых значений на соответствие условиям
                        print('Отношение должно быть целым числом от 1 до 9 ', ratio)
                    else:
                        table[i][j] = ratio
    for i in range(n):
        for j in range(n):
